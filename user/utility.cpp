...
void SMAC_OnCheatDetected(PlayerControl* pCtrl, std::string reason) {
	...
	if (IsHost()) {
		...
		case 1:
			...
			break;
		case 2:
			if (IsInLobby()) State.lobbyRpcQueue.push(new ATB(*Game::pLocalPlayer, pCtrl, 1));
			if (IsInGame()) State.rpcQueue.push(new ATB(*Game::pLocalPlayer, pCtrl, 1));
			State.SMAC_AttemptBanLobby.push_back(pCtrl->fields.PlayerId);
			break;
		}
	}
}
