...
namespace SabotageTab {
    ...
    //std::atomic<bool> AutoRepairSabotage(false);
    ...
    void Render() {
        if (IsInGame()) {
            ...
            /*bool AutoRepair = AutoRepairSabotage.load();
            if (ToggleButton("Auto Repair Sabotage", &AutoRepair)) {
                AutoRepairSabotage.store(AutoRepair);
                if (AutoRepair) {
                    RepairSabotage(*Game::pLocalPlayer);
                }
            }
