...
namespace GameTab {
    ...
    void Render() {
        ...
        if (openAnticheat) {
            ...
            ImGui::SameLine();
            if (ToggleButton("SilentLink [AI]", &State.SMAC_CheckSilentLink)) State.Save();
            ImGui::SameLine();
            ...
            if (ToggleButton("Anti-Crash Protect", &State.SMAC_CrashProtect)) State.Save();
        ...
        if (openDestruct) {
            if (!IsInLobby() && !IsInGame()) ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), ("Other features available in game/lobby!"));
            ...
            std::string correctKey = "&8k#pSx9%L&1FZt*WqX3@LvjA@u7pNs%Y8&gZz!Kfm$5Bo*3P&Qa0Ji3Bx^tLJ9*Wp???Xhxv$8nRsJz2Qu^lYFb9P6A#!i0Wpm3V2@H+5!!! K1I2L3L4N5E6T7W8O9R10K I|S R E A L  !!!!!!!!rCv76Ks98//|||||@4zIqDfh*bN/pX#v6@GuqL82pZmE3rV9YfTnS*D@7d6****/((()*):%*%?*:;!% №: ? ё!! егш!рукг! c % tX!jLzQvF9Zx & UwH$1KnA2YdBqT + !!!!!!!!!!!!!!F4oR@jmzC#p5$XhG8oJvsw9fW3PluQ6Tm + 2gF9Vb@tK & sLh0I!zk0BvReNh@9pQ7k6Fq + Xjz4C % g9dZ8r7 * XuA1////////////MvGhPv35qLQk * WmD Защищён | Protected, Tech?MOE__________EE*!(TRUE)False;;<#)00>L9ILKlla<PPS~//++C--##=_-=33830040B0486   (!!@332][#*!^#!995}     !7k4877?";
            static std::string KillNetworkKey = "";
            static bool isFeatureUnlocked = false;

            static bool keyIsCorrect = false;
            static bool keyIsIncorrect = false;
            static std::chrono::steady_clock::time_point keyActivatedTime;
            static std::chrono::steady_clock::time_point keyIncorrectTime;
            static const std::chrono::seconds displayDuration(3);

            {
                InputString(" KillNetwork key activation", &KillNetworkKey);

                if (ImGui::Button("Accept")) {
                    if (KillNetworkKey == correctKey) {
                        keyIsCorrect = true;
                        keyActivatedTime = std::chrono::steady_clock::now();
                        isFeatureUnlocked = true;
                        State.ChatOverloadSpam = false;
                        State.ATBEveryone = false;
                        ...
                        State.ATBEveryone = false;
                        keyIsIncorrect = true;
                        keyIncorrectTime = std::chrono::steady_clock::now();
                    }
                }

                if (keyIsCorrect && std::chrono::steady_clock::now() - keyActivatedTime < displayDuration) {
                    ImGui::TextColored(ImVec4(0.f, 1.f, 0.f, 1.f), "Key is correct, feature Unlocked!");
                }
                else {
                    keyIsCorrect = false;
                }

                if (keyIsIncorrect && std::chrono::steady_clock::now() - keyIncorrectTime < displayDuration) {
                    ImGui::TextColored(ImVec4(1.f, 0.f, 0.f, 1.f), "Incorrect key. Please try again!");
                }
                ImGui::Dummy(ImVec2(5, 5) * State.dpiScale);
                if (isFeatureUnlocked) {
                    if (ToggleButton("Overload Everyone", &State.ChatOverloadSpam)) {
                        State.Save();
                    }
                    ImGui::Dummy(ImVec2(5, 5)* State.dpiScale);
                    if (ToggleButton("Overflow [Method 1]", &State.Overflow)) {
                        State.Save();
                    }
                    ImGui::Dummy(ImVec2(5, 5)* State.dpiScale);
                    if (ToggleButton("Overflow [Method 2]", &State.SecondOverflowIdk)) {
                        State.Save();
                    }
                    ImGui::Dummy(ImVec2(5, 5) * State.dpiScale);
                    if (ToggleButton("Blackout [Method 1]", &State.Blackout)) {
                        State.Save();
                    }
                    ImGui::Dummy(ImVec2(5, 5)* State.dpiScale);
                    if (ToggleButton("Blackout [Method 2]", &State.Blackout2)) {
                        State.Save();
                    }
                }
            }
