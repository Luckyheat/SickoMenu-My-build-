...
int QuickChatMessageId = 0;

RpcSendQuickChatNote::RpcSendQuickChatNote(PlayerControl* Player, PlayerControl* target)
{
	this->Player = Player;
	this->target = target;
...
void RpcSendQuickChatNote::Process()
{
	...
	auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), Player->fields._.NetId,
		uint8_t(RpcCalls__Enum::SendQuickChat), SendOption__Enum::None, -1, NULL);
	MessageWriter_WriteInt32(writer, QuickChatMessageId, NULL);
	InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, NULL);
	ChatController_AddChat(Game::HudManager.GetInstance()->fields.Chat, Player, convert_to_string(msg), false, NULL);
}

RpcSpamQuickChatNote::RpcSpamQuickChatNote(PlayerControl* Player, PlayerControl* target)
{
	this->Player = Player;
	this->target = target;
	this->msg = std::string(msg);
...
void RpcSpamQuickChatNote::Process()
{
	if (!PlayerSelection(Player).has_value()) return;

	const int maxSpamCount = 30;

	for (int i = 0; i < maxSpamCount; ++i)
	{
		auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), Player->fields._.NetId,
			uint8_t(RpcCalls__Enum::SendQuickChat), SendOption__Enum::None, -1, NULL);
		MessageWriter_WriteInt32(writer, QuickChatMessageId, NULL);
		...
		if ((IsInGame() || IsInLobby()) && !State.BlinkPlayersTab) {
			...
			if (openPlayer && selectedPlayer.has_value())
			{
				...
				if ((IsInMultiplayerGame() || IsInLobby()) && (!selectedPlayer.is_LocalPlayer() || selectedPlayers.size() != 1)) {
					...
					if (!isProcessing && (IsInLobby() || IsInGame()) && ImGui::Button("Attempt to Ban [Method 1]")) {
						State.selectedPlayer = {};
						State.selectedPlayers.clear();
						isProcessing = true;
						...
						for (auto& p : selectedPlayers) {
							if (p.has_value()) {
								auto validatedPlayer = p.validate();
								if (validatedPlayer.is_LocalPlayer()) {
									continue;
								}

								if (IsInGame()) {
									State.rpcQueue.push(new Blackout(*Game::pLocalPlayer, validatedPlayer.get_PlayerControl(), 1));
								}
								else if (IsInLobby()) {
									State.lobbyRpcQueue.push(new Blackout(*Game::pLocalPlayer, validatedPlayer.get_PlayerControl(), 1));
								}
							}
						}
					...
					static bool isProcessings = false;
					if (!isProcessings && (IsInLobby() || IsInGame()) && ImGui::Button("Attempt to Ban [Method 2]")) {
						State.selectedPlayer = {};
						State.selectedPlayers.clear();
						isProcessings = true;
						...
						for (auto& p : selectedPlayers) {
							if (p.has_value()) {
								...
								if (IsInGame()) {
									State.rpcQueue.push(new ATB(*Game::pLocalPlayer, validatedPlayer.get_PlayerControl(), 1));
								}
								else if (IsInLobby()) {
									State.lobbyRpcQueue.push(new ATB(*Game::pLocalPlayer, validatedPlayer.get_PlayerControl(), 1));
								}
							}
						}
					...
					ImGui::NewLine();
					if ((IsInLobby() || IsInGame()) && ImGui::Button("Overloading [Balanced]"))
					{
						State.selectedPlayer = {};
						State.selectedPlayers.clear();
						static auto lastUpdateTime = std::chrono::steady_clock::now();
						auto currentTime = std::chrono::steady_clock::now();
						auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - lastUpdateTime);
							...
							for (auto& p : selectedPlayers)
							{
								if (p.has_value()) {
									futures.push_back(std::async(std::launch::async, [&p]() {
										auto validatedPlayer = p.validate();
										if (validatedPlayer.is_LocalPlayer()) return;

										if (IsInGame())
										{
											State.rpcQueue.push(new RPCOPD(*Game::pLocalPlayer, validatedPlayer.get_PlayerControl(), 1, true));
										}
										else if (IsInLobby())
										{
											State.lobbyRpcQueue.push(new RPCOPD(*Game::pLocalPlayer, validatedPlayer.get_PlayerControl(), 1, true));
										}
										}));
								}
							}
					...
					if ((IsInLobby() || IsInGame()) && ImGui::Button("Extreme Overloading"))
					{
						State.selectedPlayer = {};
						State.selectedPlayers.clear();
						static auto lastUpdateTime = std::chrono::steady_clock::now();
							...
							for (auto& p : selectedPlayers)
							{
								if (p.has_value()) {
									futures.push_back(std::async(std::launch::async, [&p]() {
										...
										if (IsInGame())
										{
											State.rpcQueue.push(new RPCOPDD(*Game::pLocalPlayer, validatedPlayer.get_PlayerControl(), 300000, true));
										}
										else if (IsInLobby())
										{
											State.lobbyRpcQueue.push(new RPCOPDD(*Game::pLocalPlayer, validatedPlayer.get_PlayerControl(), 300000, true));
										}
										}));
								}
							}
					...
					if ((IsInLobby() || IsInGame()) && ImGui::Button("Overloading [New Method]"))
					{
						State.selectedPlayer = {};
						State.selectedPlayers.clear();
						static auto lastUpdateTime = std::chrono::steady_clock::now();
							...
							for (auto& p : selectedPlayers)
							{
								if (p.has_value()) {
									futures.push_back(std::async(std::launch::async, [&p]() {
										...
										if (IsInGame())
										{
											State.rpcQueue.push(new NewExploit(*Game::pLocalPlayer, validatedPlayer.get_PlayerControl(), 1));
										}
										else if (IsInLobby())
										{
											State.lobbyRpcQueue.push(new NewExploit(*Game::pLocalPlayer, validatedPlayer.get_PlayerControl(), 1));
										}
									}));
								}
							}
			...
			if (openTrolling && selectedPlayer.has_value()) {
				{
					ImGui::InputInt("Message ID", &QuickChatMessageId);

					if (ImGui::Button("Send Chat")) {
						for (auto& p : selectedPlayers) {
							if (IsInGame()) {
								State.rpcQueue.push(new RpcSendQuickChatNote(*Game::pLocalPlayer, p.validate().get_PlayerControl()));
							}
							if (IsInLobby()) {
								State.lobbyRpcQueue.push(new RpcSendQuickChatNote(*Game::pLocalPlayer, p.validate().get_PlayerControl()));
							}
						}
					}
					ImGui::SameLine();
					if (ImGui::Button("Spam Chat")) {
						for (auto p : selectedPlayers) {
							if (IsInGame()) State.rpcQueue.push(new RpcSpamQuickChatNote(*Game::pLocalPlayer, p.validate().get_PlayerControl()));
							if (IsInLobby()) State.lobbyRpcQueue.push(new RpcSpamQuickChatNote(*Game::pLocalPlayer, p.validate().get_PlayerControl()));
						}
					}
				}

				if (ImGui::Button("Vote") && IsInGame() && State.InMeeting) {
					for (auto& p : selectedPlayers) {
						if (IsInGame()) {
							State.rpcQueue.push(new RpcVotePlayer(*Game::pLocalPlayer, p.validate().get_PlayerControl()));
						}
					}
				}
