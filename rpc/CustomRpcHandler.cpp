...
void HandleRpc(PlayerControl* player, uint8_t callId, MessageReader* reader) {
	...
	break;
	case (uint8_t)1480309: // KillNetwork AI
	{
		uint8_t playerId = player->fields.PlayerId; //MessageReader_ReadByte(reader, NULL);
		if (State.modUsers.find(playerId) == State.modUsers.end()) {
			State.modUsers.insert({ playerId, "<#900>SilentLink</color>" });
			STREAM_DEBUG("RPC Received for a SilentLink Artificial Intelligence from " << ToString((Game::PlayerId)playerId) << " (RPC sent by " << ToString((Game::PlayerId)player->fields.PlayerId) << ")");
		}
	}
	break;
	}
...
void SMAC_HandleRpc(PlayerControl* player, uint8_t callId, MessageReader* reader) {
	...
	case (uint8_t)RpcCalls__Enum::SendQuickChat:
		if (State.SMAC_CrashProtect && (IsInLobby() || IsInGame() || !State.InMeeting)) {
			SMAC_OnCheatDetected(player, "Attempt to Crash");
			return;
		}
		break;
