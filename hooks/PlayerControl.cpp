	...
	try {
		...
		if ((IsInGame() || IsInLobby())) {
			...
			if (State.ModDetection) {
				...
				if (State.SilentLink && __this == *Game::pLocalPlayer && !IsInGame()) {
					if (State.rpcCooldown == 0) {
						MessageWriter* writer = InnerNetClient_StartRpc((InnerNetClient*)(*Game::pAmongUsClient), __this->fields._.NetId, (uint8_t)1480309, (SendOption__Enum)1, NULL);
						MessageWriter_EndMessage(writer, NULL);
						State.rpcCooldown = 15;
					}
					else {
			...
			if (State.ShowPlayerInfo && IsInLobby() && !State.PanicMode)
			{
				...
				if (State.ModDetection) {
					...
					if (State.SilentLink) localPlayerMod = "<#900>SilentLink AI]<#0000>";
				}
			...
			if (!State.PanicMode) {
				...
				if (State.ChatOverloadSpam && (IsInGame() || IsInLobby())) {
					auto player = State.playerToChatAs.has_value() ? State.playerToChatAs.validate().get_PlayerControl() : *Game::pLocalPlayer;
					auto allPlayers = GetAllPlayerControl();
					bool sendToEveryone = State.ChatSpamEveryone;
					if (State.ChatOverloadSpamMode == 1) {
						for (auto& p : allPlayers) {
							if (p != player || sendToEveryone) {
								try {
									auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), player->fields._.NetId, uint8_t(RpcCalls__Enum::SendQuickChat), SendOption__Enum::None, -1, nullptr);
									MessageWriter_WriteInt32(writer, 179202, NULL);
									InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, nullptr);
								}
								catch (const std::exception& e) {
									std::cerr << "Error sending RPC message to player " << p->fields._.NetId << ": " << e.what() << std::endl;
								}
							}
						}
					}
				...
				if (State.Overflow && (IsInGame() || IsInLobby())) {
					auto allPlayers = GetAllPlayerControl();
					auto localPlayerNetId = (*Game::pLocalPlayer)->fields._.NetId;

					for (auto& p : allPlayers) {
						try {
							if (p->fields._.NetId == localPlayerNetId) {
								continue;
							}

							auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), (*Game::pLocalPlayer)->fields._.NetId, uint8_t(RpcCalls__Enum::StartMeeting), SendOption__Enum::None, p->fields._.OwnerId, NULL);
							InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, NULL);
				...
				if (State.SecondOverflowIdk && (IsInGame() || IsInLobby())) {
					auto allPlayers = GetAllPlayerControl();
					auto localPlayerNetId = (*Game::pLocalPlayer)->fields._.NetId;
					...
					for (auto& p : allPlayers) {
						try {
							...
							auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), (*Game::pLocalPlayer)->fields._.NetId, uint8_t(RpcCalls__Enum::CloseDoorsOfType), SendOption__Enum::None, p->fields._.OwnerId, NULL);
							InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, NULL);
				...
				if (State.Blackout && (IsInGame() || IsInLobby())) {
					auto allPlayers = GetAllPlayerControl();
					auto localPlayerNetId = (*Game::pLocalPlayer)->fields._.NetId;
					...
					for (auto& p : allPlayers) {
						try {
							...
							auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), (*Game::pLocalPlayer)->fields._.NetId, uint8_t(RpcCalls__Enum::CheckName), SendOption__Enum::None, p->fields._.OwnerId, NULL);
							MessageWriter_WriteString(writer, convert_to_string("Ｌｕｃｋｙｈｅａｔ"), NULL);
							InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, NULL);
				...
				if (State.Blackout2 && (IsInGame() || IsInLobby())) {
					auto allPlayers = GetAllPlayerControl();
					auto localPlayerNetId = (*Game::pLocalPlayer)->fields._.NetId;
					...
					for (auto& p : allPlayers) {
						try {
							...
							auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), (*Game::pLocalPlayer)->fields._.NetId, uint8_t(RpcCalls__Enum::CheckColor), SendOption__Enum::None, p->fields._.OwnerId, NULL);
							MessageWriter_WriteString(writer, convert_to_string("Ｌｕｃｋｙｈｅａｔ"), NULL);
							InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, NULL);
