...
void dVersionShower_Start(VersionShower* __this, MethodInfo* method) {
	...
	const auto& versionText = !State.PanicMode && !State.HideWatermark ? std::format("<size=75%>{}{} ~ <#0f0>Sicko</color><#f00>Menu</color> <#fb0>{}</color> <#F00>[KillNetwork Mode]</color> by <#39f>g0aty</color></color></size>",
...
void dPingTracker_Update(PingTracker* __this, MethodInfo* method) {
	...
	try {
		if (!IsStreamerMode() && (!State.PanicMode || State.TempPanicMode)) {
			...
			std::string watermarkText = std::format("<size={}%><#0f0>Sicko</color><#f00>Menu</color> <#fb0>{}</color> <#F00>[KillNetwork Mode]</color> by <#39f>g0aty</color> ~ ", spectating == "" ? 100 : 50, State.SickoVersion);
